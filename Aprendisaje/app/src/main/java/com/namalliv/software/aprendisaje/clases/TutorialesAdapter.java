...
public class TutorialesAdapter extends ArrayAdapter<Tutorial> {
    List<Tutorial> mProgramaLista;
    ...
    public TutorialesAdapter(Context context, List<Tutorial> objects) {
    ...
    public View getView(int posistion, View converView, ViewGroup parent){
        ...
        Tutorial programa = mProgramaLista.get(posistion);
        char inicial = programa.getTutorial().charAt(0);
        inicial = Character.toUpperCase(inicial);
        //numero.setText(Integer.toString(programa.getId()));
        numero.setText(inicial+"");
        //actividad.setText(inicial + " " +programa.getTutorial().substring(1));
        actividad.setText(programa.getTutorial());
