...
public class ProgramasAdapter extends ArrayAdapter {
    List<Programa> mProgramaLista;
    ...
    public ProgramasAdapter(Context context, List<Programa> objects) {
    ...
    public View getView(int posistion, View converView, ViewGroup parent){
        ...
        Programa programa = mProgramaLista.get(posistion);
        char inicial = programa.getPrograma().charAt(0);
        inicial = Character.toUpperCase(inicial);

        //numero.setText(Integer.toString(programa.getId()));
        numero.setText(inicial+"");
        actividad.setText(programa.getPrograma());
